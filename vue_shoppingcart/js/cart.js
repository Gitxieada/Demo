var vm = new Vue({
    ...
    filters: { //局部过滤器
        //用于格式化，vue方法内的是局部过滤器,在dom中用“ | formatMoney”挂载
    ...
    mounted: function () { //代替1.0版本中的ready函数
        ...
        //     vm.loadList();   //通过$nextTick()函数，vm可以等价于this
        // });
        //使用 mounted 并不能保证钩子函数中的 this.$el 在 document 中。为此还应该引入 Vue.nextTick/vm.$nextTick
    },
    methods: {
        cartView: function () {
            ...
            // });
            // let _this = this;   //ES6特性，声明变量用let
            this.$http.get("data/cartData.json", {
                id: "123"
            }).then(res => { //ES6特有的箭头函数，res相当于回调的参数
                //=>相当于function，{}里是函数体
